...
function NavLinkItems (){
    ...
    return (
        <ul className="nav-menu flex flex-col font-medium p-4 md:p-0 mt-4 border  rounded-lg md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 ">
            {
                itemsNavLink.map( (link, index) => 
                    <li key={index}> 
                        <NavLink to={link.href}
                            className="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-violet-700 md:p-0 dark:text-gray-900 md:dark:hover:text-gray-900 dark:hover:bg-gray-700 dark:hover:text-black  md:dark:hover:bg-transparent"
                        >
                            {link.name}
                        </NavLink>
                    </li>
